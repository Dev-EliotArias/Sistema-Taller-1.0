...
public class OrdenTrabajoImpl implements OrdenTrabajoService {
	...
	public List<OrdenTrabajoDTO> getAllOrdenesTrabajo() {
		...
		return ordenesTrabajo.stream().map(ordenTrabajo -> convertToDTO(ordenTrabajo)).collect(Collectors.toList());
	}
	...
	public Optional<OrdenTrabajoDTO> getOrdenTrabajoById(Long id) {
		...
		return Optional.ofNullable(ordenTrabajoRepository.findById(id)).map(ordenTrabajoOptional -> {
			if (ordenTrabajoOptional.isPresent()) {
				System.out.println(ordenTrabajoOptional.get().getEstado());
				return convertToDTO(ordenTrabajoOptional.get());
			} else {
	...
	public Optional<OrdenTrabajoDTO> createOrdenTrabajo(OrdenTrabajoDTO nuevaOrdenTrabajoDTO) {
		...
		List<Long> serviciosIds = nuevaOrdenTrabajoDTO.getServiciosId();
		System.out.println("Servicios IDs: " + serviciosIds);

		Vehiculo vehiculoOrdenT = vehiculoRepository.findById(nuevaOrdenTrabajoDTO.getVehiculoId())
				.orElseThrow(() -> new NotFoundException("Vehiculo no Encontrado"));
		List<Servicios> serviciosOrdenT = serviciosRepository
				.findAllById(nuevaOrdenTrabajoDTO.getServiciosId().stream().collect(Collectors.toList()));
		Tecnico tecnicoOrdenT = tecnicoRepository.findById(nuevaOrdenTrabajoDTO.getTecnicoId())
				.orElseThrow(() -> new NotFoundException("Tecnico not found"));

		if (nuevaOrdenTrabajoDTO == null || vehiculoOrdenT == null || tecnicoOrdenT == null
				|| serviciosOrdenT.isEmpty()) {
			return Optional.empty();
//			throw new BadRequestException()
		}

		System.out.println(serviciosIds);
		

		OrdenTrabajo nuevaOrdenTrabajo = OrdenTrabajo.builder().vehiculo(vehiculoOrdenT).servicios(serviciosOrdenT)
				.tecnico(tecnicoOrdenT).fechaIngreso(nuevaOrdenTrabajoDTO.getFechaIngreso())
				.fechaSalida(nuevaOrdenTrabajoDTO.getFechaSalida()).costoTotal(nuevaOrdenTrabajoDTO.getCostoTotal())
				.comentario(nuevaOrdenTrabajoDTO.getComentario()).estado(nuevaOrdenTrabajoDTO.getEstado()).build();

		OrdenTrabajo ordenTrabajoGuardada = ordenTrabajoRepository.save(nuevaOrdenTrabajo);
		
		// Verificando que los servicios se guardaron correctamente
	    List<Servicios> serviciosGuardados = ordenTrabajoGuardada.getServicios();
	    for (Servicios servicio : serviciosGuardados) {
	        System.out.println("Servicio guardado: " + servicio.getNombre());
	    }
	...
	// servicios.stream().map(servicio -> modelMapper.map(servicio,
	// ServiciosDTO.class)).collect(Collectors.toList());
	...
	public OrdenTrabajoDTO updateOrdenTrabajo(Long id, OrdenTrabajoDTO ordenTrabajoActualizadaDTO) {
		...
		Vehiculo vehiculoOrdenT = vehiculoRepository.findById(ordenTrabajoActualizadaDTO.getVehiculoId())
				.orElseThrow(() -> new NotFoundException("Vehiculo not found"));
		List<Servicios> serviciosOrdenT = serviciosRepository
				.findAllById(ordenTrabajoActualizadaDTO.getServiciosId().stream().collect(Collectors.toList()));
		Tecnico tecnicoOrdenT = tecnicoRepository.findById(ordenTrabajoActualizadaDTO.getTecnicoId())
				.orElseThrow(() -> new NotFoundException("Tecnico not found"));

		if (ordenTrabajoActualizadaDTO == null || vehiculoOrdenT == null || tecnicoOrdenT == null
				|| serviciosOrdenT.isEmpty()) {
			return null;
		}
	...
	public OrdenTrabajoDTO convertToDTO(OrdenTrabajo ordenTrabajo) {
		...
		List<Long> serviciosIds = ordenTrabajo.getServicios().stream().map(Servicios::getId)
				.collect(Collectors.toList());

		return OrdenTrabajoDTO.builder().id(ordenTrabajo.getId()).vehiculoId(ordenTrabajo.getVehiculo().getId())
				.fechaIngreso(ordenTrabajo.getFechaIngreso()).fechaSalida(ordenTrabajo.getFechaSalida())
				.serviciosId(serviciosIds).costoTotal(ordenTrabajo.getCostoTotal())
				.comentario(ordenTrabajo.getComentario()).estado(ordenTrabajo.getEstado())
				.tecnicoId(ordenTrabajo.getTecnico().getId()).build();
	}
